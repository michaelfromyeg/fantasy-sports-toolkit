...
`fantasy-sports-toolkit` is a [Weft](https://github.com/michaelfromyeg/weft) marketplace
(`fantasy-sports-toolkit`, namespace `dev.fantasylab`). You write provider-agnostic skills,
commands, an agent, and MCP server definitions one time, and Weft compiles them
...
`out/` and `.claude/` are generated by Weft (`weft build` / `weft install`) and
...
including both football and basketball lineup cases. Run with `weft eval` against
the Claude harness.
...
## Build & install with the Weft CLI
...
weft build . --out out --target claude,codex
...
weft install . --only set-lineup      # install a single component piecemeal

# Run a skill's eval suite (currently targeted at the claude harness)
weft eval plugins/fantasy-manager
```

`weft build`'s default output directory is `.weft-out`; this repo uses `out/`, so
...
Authored once, compiled everywhere by **Weft** --
<https://github.com/michaelfromyeg/weft> (CLI:
[`@michaelfromyeg/weft-cli`](https://www.npmjs.com/package/@michaelfromyeg/weft-cli)).
